...
module.exports.run = async (client, message, args) => {
      ...
      if (!uye) return message.channel.send(aiasembed.setDescription("Ses odasına gidilecek üyeyi belirtmelisin!")).then(x => x.delete({timeout: 5000}));
      ...
      if (message.member.hasPermission("ADMINISTRATOR")) {
        ...
        message.channel.send(`${uye}`, {embed: aiasembed.setAuthor(uye.displayName, uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${message.author} senin ses kanalına girmek için izin istiyor! Onaylıyor musun?`)}).then(async msj => {
          await msj.react('✅');
